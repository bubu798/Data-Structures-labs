                     ...
                     int cars_no, int platform_b, int pos_b)
{
    ...
    if (i != pos_a)
        return; // cazul in care primul vagon care trebuie mutat nu exista
    ...
    if (i != pos_b)
        return; // cazul in care incearca sa adauge vagoane pe o pozitie inexistenta
    ...
    if (count >= cars_no)
        ...
        {
            if (prev_a != NULL)
                prev_a->next = ptr->next; // in caz ca primul vagon care trebuie mutat nu e primul din tren
            ...
            if (prev_b != NULL)
                prev_b->next = ptr; // daca pozitia pe care vagonul trebuia mutat nu e prima, atunci leaga vagonul din spate de acesta
            if (prev_a == NULL)
                station->platforms[platform_a]->train_cars = curr; // daca pozitia vagonului care trebuie mutat e prima, atunci urmatorul vagon
                                                                   // va deveni primul (asfel eliminandu-l din tren)
            if (prev_b == NULL)
                station->platforms[platform_b]->train_cars = ptr; // daca pozitia unde vagonul trebuie mutat este prima atunci ptr devine primul vagon
            if (prev_b != NULL)
                prev_b = prev_b->next;
            else
                prev_b = ptr;
...
int find_heaviest_sequence_train(TrainStation *station, int cars_no, TrainCar **start_car)
{
    ...
    for (int i = 0; i < station->platforms_no; ++i)
    {
        ...
        {
            if (station->platforms[i])
                curr = station->platforms[i]->train_cars;
            while (curr != NULL)
            {
                if (count >= cars_no)
                {
